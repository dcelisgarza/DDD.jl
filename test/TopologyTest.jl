...
@testset "Coarsen and Refine network" begin
    ...
    calcSegForce!(dlnParams, matParams, network4)

    network5 = deepcopy(network4)
    coarsenNetwork!(dlnParams, matParams, network5)

    network6 = deepcopy(network5)
    refineNetwork!(dlnParams, matParams, network6)
    ...
    segForce2 = [0.769835430141919   0.769835430141919   0.695853577592157
    ...
    @test network6.numNode == numNode
    @test network6.numSeg == numSeg
    @test network6.links[:, 1:numSeg]' == links
    @test isapprox(network6.slipPlane[:, 1:numSeg]', slipPlane)
    @test isapprox(network6.bVec[:, 1:numSeg]', bVec)
    @test network6.label[1:numNode] == label
    @test isapprox(network6.coord[:, 1:numNode]', coord)
    @test isapprox(network6.nodeVel[:, 1:11]', nodeVel[1:11,:], rtol=1e-3)
    @test isapprox(network6.nodeVel[:, 11:13]', nodeVel[11:13,:], rtol=0.5)
    @test isapprox(network6.nodeVel[:, 14:15]', nodeVel[14:15,:], rtol=1)
    @test isapprox(network6.nodeVel[:, 16:numNode]', nodeVel[16:end,:], rtol = 0.7)
    @test network6.connectivity[:, 1:numNode]' == connectivity
    network6.linksConnect[:, 1:numSeg]' == linksConnect
    @test isapprox(network6.segForce[:, 1, 1:numSeg]', segForce1)
    @test isapprox(network6.segForce[:, 2, 1:numSeg]', segForce2)
